...
const parsePackageJson = (packageJson) => {
    ...
    let binPath = path.join("bin", "supagrate");
    if (platform == "windows") {
        binPath += ".exe";
    }
...
const errGlobal = `Installing Supagrate CLI as a global module is not supported.
Please use one of the supported package managers: https://github.com/supagrate/cli#installation
`;
...
async function main() {
    ...
    console.info("Installed Supagrate CLI successfully");
}
